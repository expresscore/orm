...
class OrmService
{
    ...
    private static function readParameters(array $arguments) : array
    {
        ...
        $config['mode'] = 'prod';
        $config['sqlAdapterClass'] = 'expresscore\orm\MySQLAdapter';
    ...
    public static function importAction(array $config, array $arguments) : void
    {
        switch ($arguments[2]) {
            case 'fixtures':
                ...
                break;
            default:
                throw new Exception('Unknown import action argument: ' . $arguments[2]);
        }
    }
    ...
    private static function generateMigrationAction(array $config) : void
    {
        ...
        if (!empty($migrationQueries)) {
            ...
            $fileContent .= 'use expresscore\orm\AbstractMigration;' . PHP_EOL . PHP_EOL;
